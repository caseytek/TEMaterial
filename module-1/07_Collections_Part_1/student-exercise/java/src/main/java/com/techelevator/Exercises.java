...
//		String[] animalsArray = animals.toArray(new String[animals.size()]);
	...
	public List<String> reverseList(List<String> stringList) {
		...
		while (stringStack.size() > 0) {
			reverseStringList.add(stringStack.pop());		
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (int x: intArray) {
			Double newDouble = new Double(x/2);
	...
	** INTERVIEW QUESTION **
	...
	public List<String> fizzBuzzList(Integer[] integerArray) {
		...
		for(Integer number: integerArray) {
			if (number % 3 == 0 && number % 5 == 0) {
				...
				fizzBuzzString.add(stringNumber);
			}
		}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		List <Integer> boardingGateList = new LinkedList<>();
		for (Integer number: seatNumberList) {
			if (number >= 1 && number <= 10) {
				boardingGateList.add(number);
			}
		} 
		for (Integer number: seatNumberList) {
