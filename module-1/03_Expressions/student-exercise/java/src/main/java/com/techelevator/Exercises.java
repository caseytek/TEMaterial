...
public class Exercises {
	...
	public boolean hasTeen(int a, int b, int c) {
		boolean result = false;
		if ((a >= 13 && a <= 19) || (b >= 10 && b <= 20) || (c >= 10 && c <= 20)) {
			result = true;
		}
		return result;
	}
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
	 boolean result = false;
	 boolean weekdaySuccess = (cigars >= 40 && cigars <= 60);
	 boolean weekendSuccess = (cigars >= 40);
	 if ((isWeekend && weekendSuccess) || (!isWeekend && weekdaySuccess)) {
		 result = true;
	 }
	 return result;
	}
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		boolean result = false;
		boolean notSummer = (temp >=60 && temp <= 90);
		boolean summer = (temp >=60 && temp <= 100);
		if ((notSummer && !isSummer) || (summer && isSummer)) {
			result = true;
		} 
		return result;
	}
	...
	public int sortaSum(int a, int b) {
		int result = 0;
		boolean returnTwenty = (((a + b) >= 10) && ((a + b) <= 19));
		if (returnTwenty) {
			result = 20;
		} else {
			result = (a+b);
	...
	public String alarmClock(int day, boolean vacation) {
		String result = "";
		boolean weekdayNumbers = (day >= 1 && day <=5);
		boolean weekendNumbers = (day == 0 || day == 6);
		if (vacation) {
			if (weekdayNumbers) {
				result = "10:00";
			} else if (weekendNumbers) {
				result = "off";
			}
		} 
		if (!vacation) {
	...
	public int teenSum(int a, int b) {
		int result = (a+b);
		boolean teenNumber = ((a >= 13 && a <= 19) || (b >= 13 && b <= 19));
		if (teenNumber) {
			result = 19;
		} return result;
	}
